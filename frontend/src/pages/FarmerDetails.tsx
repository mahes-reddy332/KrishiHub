    ...
    if (loading) return (
        <Layout>
            <div className="container flex items-center justify-center h-[70vh]">
                <div className="text-xl animate-pulse">{t("loadingFarmerDetails")}</div>
            </div>
        </Layout>
    );
      ...
      <Layout>
        <div className="container py-8 px-4">
          <div className="farmer-details-container glass-panel">
            <h2 className="text-2xl font-semibold mb-6">{t("farmerDetails")}</h2>
            <div className="farmer-card">
              <div className="farmer-info w-full">
                <p>
                  <strong>{t("nameLabel")}</strong> {farmer?.name}
                </p>
                <p>
                  <strong>{t("areaLabel")}</strong> {farmer?.area}
                </p>
                <p>
                  <strong>{t("landAreaLabel")}</strong> {farmer?.landArea} acres
                </p>
                <p>
                  <strong>{t("phoneLabel")}</strong> {farmer?.phone}
                </p>
                <p>
                  <strong>{t("emailLabel")}</strong> {farmer?.email}
                </p>
                <p>
                  <strong>{t("dateOfStarting")}</strong>{" "}
                  {formattedDate?.toLocaleDateString()}
                </p>
                <p>
                  <strong>{t("selectedCropLabel")}</strong>{" "}
                ...
                formattedDate && (
                  <div className="crop-details w-full mt-8">
                    <h3 className="text-xl font-medium flex items-center gap-2">
                      <Sprout className="h-5 w-5" /> {t("cropSchedule")}{" "}
                      {farmer?.selectedCrop}
                    </h3>

                    <div className="mt-4">
                      <h4 className="text-lg font-medium flex items-center gap-2">
                        <Calendar className="h-4 w-4" /> {t("dailyTasks")}
                      </h4>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                      <div className="bg-white bg-opacity-20 rounded-lg p-4">
                        <h4 className="text-lg font-medium flex items-center gap-2 mb-3">
                          <Droplets className="h-4 w-4" /> {t("waterSchedule")}
                        </h4>
                        <ul className="list-disc pl-6">
                      ...
                      <div className="bg-white bg-opacity-20 rounded-lg p-4">
                        <h4 className="text-lg font-medium flex items-center gap-2 mb-3">
                          <Calendar className="h-4 w-4" /> {t("growthStages")}
                        </h4>
                        <ul className="list-disc pl-6">
                      ...
                      <div className="bg-white bg-opacity-20 rounded-lg p-4 md:col-span-2">
                        <h4 className="text-lg font-medium flex items-center gap-2 mb-3">
                          <Calendar className="h-4 w-4" /> {t("fertilizerSchedule")}
                        </h4>
                        <ul className="list-disc pl-6">
                    ...
                    <div className="mt-6 p-4 bg-blue-500 bg-opacity-20 rounded-lg">
                      <p className="font-medium">
                        {t("totalCropDuration")} {cropInfo.duration} {t("days")}
                      </p>
                    </div>
                    <div className="mt-4">
                      <button
